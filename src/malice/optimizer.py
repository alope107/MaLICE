...
class MaliceOptimizer(object):
    ...
    def fitness(self, params=None):
        if self.mode == 'ml_optimization':
            ...
            Kd_exp, koff_exp, dR2, amp, i_noise, cs_noise = self.l1_model[:self.gvs]
            ...
            Kd_exp, koff_exp, dR2, amp, i_noise, cs_noise = self.ml_model[:self.gvs]
            residue_params = self.reference.copy()
            ...
            concs = self.data[['tit','obs']].drop_duplicates()
        ...
        obsconc = list(self.data.obs.drop_duplicates())
        if len(obsconc) > 1:
            amp = amp*(self.data.obs/np.mean(obsconc))
